...
### v2.1.3 (2026-01-27) - 角色权限与首页管理整合 🔐🏠

#### ✨ 新增功能

- 用户角色体系：`student` / `committee` / `admin`
- 管理员角色管理接口（调整用户角色、按角色筛选）
- 班级通知与班级日历的管理入口整合到首页模块
- 班委/管理员可在首页直接发布通知与新增/编辑日历事件

#### 🔧 技术改进

**后端**
- `users.role` 字段与迁移脚本
- 通知/日历写操作权限校验（仅班委/管理员）
- 登录与用户信息响应包含 `role`

**前端**
- 首页通知模块支持发布/编辑/删除
- 首页日历模块支持点击日期新增事件、编辑/删除
- 角色权限驱动的按钮显示与提示
...
**未登录用户访问验证错误**
- 修复博客详情页未登录访问时的 Pydantic 验证错误
- 修复通知详情页的相同问题
- 原因：Python `and` 运算符返回 `None` 而非 `False`
- 解决方案：使用 `is not None` 显式检查确保返回布尔值
- 详见：[FIXED_BUG.md #16](./FIXED_BUG.md#bug-16-未登录用户访问博客通知详情页时-pydantic-验证错误)
