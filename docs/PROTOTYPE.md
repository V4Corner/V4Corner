...
| 字段 | 类型 | 必填 | 验证规则 |
|------|------|------|----------|
| email | string | 是 | 邮箱格式，唯一，验证码发送目标 |
| verification_code | string | 是 | 邮箱收到的6位数字验证码 |
...
2. **获取验证码**
   - 点击"获取验证码"按钮
   - 按钮禁用，显示60秒倒计时（"60s后重试"）
   - 调用 `/api/verification/send` 接口
   - 验证码发送到用户邮箱
   - 支持模拟模式（开发测试时自动返回模拟验证码）

3. **输入验证码**
   - 用户输入邮箱收到的6位数字验证码
   - 输入框右下角显示提示："请输入邮箱收到的6位验证码"
   - 注册时自动验证验证码有效性
...
## 13. AI对话详情页 `/chat/:id`（优化版：左侧历史栏 + 单气泡生成状态）

### 页面布局

```
┌─────────────────────────────────────────────────────────┐
│ [V4Corner] [首页] [博客] [成员] [AI对话] 💬       用户   │
├─────────────────────────────────────────────────────────┤
│ ┌───────────────┐ ┌───────────────────────────────────┐ │
│ │ [新对话]      │ │ ←  机器学习入门指南              │ │
│ │               │ ├───────────────────────────────────┤ │
...
#### **整体布局**
- **应用式对话界面**：全局导航下方分为左侧历史栏和右侧对话面板，页面主体本身不滚动
- **左侧历史栏**：固定显示 `[新对话]` 和最近历史聊天记录，当前对话高亮；历史过多时仅左侧列表内部滚动
- **右侧对话面板**：顶部只保留对话标题与返回列表入口，避免中间工具栏在滚动时漂移
- **固定全局导航**：全局导航栏保持 `sticky`，长对话中不会被聊天内容滚动隐藏
- **中央内容区**：最大宽度 768px，居中显示
- **消息区独立滚动**：右侧消息区单独滚动，左侧历史栏和底部发送区不随消息滚动移动
- **自动滚动**：新消息出现时自动滚动到底部

#### **生成状态设计**

AI 回复只使用一条 assistant 消息气泡，不再额外创建独立加载框：

| 阶段 | 气泡内容 | 说明 |
|------|----------|------|
| thinking | `正在思考 ● ● ●` | 用户消息已发送，等待首个 token |
| streaming | 逐字增长的回复内容 | 同一气泡内流式更新 |
| done | 完整回复 + 操作按钮 | 显示复制、反馈等操作 |
| error | 错误提示 + 重试入口 | 不生成第二个 AI 气泡 |
...
- 👍 有帮助：标记为有帮助（反馈）
- 👎 无帮助：标记为无帮助（反馈）
- 按钮仅在 AI 回复完成且鼠标悬浮时显示，思考/生成中不显示
...
- **返回列表**：点击返回对话列表页
- **新对话按钮**：位于左侧历史栏顶部，创建新对话并跳转
- **历史聊天**：左侧栏展示最近 30 个对话，包含标题、消息数、更新时间；点击切换对话
- **删除对话**：历史项右侧显示红色垃圾桶，点击后弹出确认框；确认后调用删除接口并从列表移除
- **删除当前对话**：删除当前正在查看的对话后，优先跳转到剩余第一条历史；若无剩余历史则返回 `/chat`
- **自动命名**：新对话首次发送消息后，后端根据第一条用户消息生成 24 字以内标题；不额外调用 AI，避免增加延迟和 token 成本
...
**自动命名**：
- 用户发送第一条消息后，由后端自动生成对话标题
- 标题生成规则：清理首条用户消息空白和句末标点，截取前 24 字；不额外调用 AI
- 用户可以在菜单中手动重命名
...
- `student`：只读班级通知与日历
- `committee`：可发布/编辑/删除通知与日历事件
- `admin`：拥有班委权限，并可管理用户角色

### 5.2 通知列表页（权限入口）

**新增区域**
- 右上角增加「发布通知」按钮（仅 `committee/admin` 显示）
- `student` 用户顶部显示权限提示条，告知仅可浏览

**交互说明**
- 点击「发布通知」进入通知编辑页
- 无权限时按钮隐藏，提示条展示“需班委权限”

### 5.3 通知发布/编辑页 `/notices/new`
...
### 5.4 日历管理页 `/calendar/manage`

**功能点**
- 月视图 + 右侧事件列表
- 新增事件入口（仅 `committee/admin`）
- 点击日期展示当日事件，可编辑/删除
...
### 5.5 角色管理页 `/admin/roles`

**功能点**
- 角色筛选（全部 / student / committee / admin）
- 用户卡片显示当前角色
- 管理员可一键提升/降级角色
